...
;(async () => {
  try {
    ...
    const host = "mongodb+srv://m220student:m220password@mflix.v8r1g.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
    ...
    const predicate = {}
    const projection = {}
    const found = await mflix
    .collection("movies")

console.log("HEEEEEEEEEEEEEREEE")
    const movies = await found.find({lastupdated: {$type: 'string'}}).toArray()
    console.log("STEEEEEEEEEXXXX EMMM", movies.length)
    const moviesToMigrate = movies.map(({ _id, lastupdated }) => ({
      updateOne: {
    ...
    console.log("CURSOOOOOOOOOR", moviesToMigrate.length)

    // // TODO: Complete the BulkWrite statement below
    const modifiedCount  = await found.bulkWrite(moviesToMigrate)

    console.log("\x1b[32m", `${modifiedCount.length} documents updated`)
    await client.close()
    process.exit(0)
  } catch (e) {
    if (
